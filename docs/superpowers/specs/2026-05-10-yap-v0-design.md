...
**Build tooling:** the Xcode project is generated from `project.yml` via [XcodeGen](https://github.com/yonaskolb/XcodeGen). `yap.xcodeproj/` is `.gitignored` and regenerated locally via `./bootstrap.sh`. This keeps `project.yml` as the single source of truth, avoids merge conflicts in `project.pbxproj`, and lets agents touch only YAML when modifying the app target.
...
│   └── yap.entitlements      # generated by XcodeGen from project.yml
├── Packages/
...
├── README.md
├── LICENSE                   # MIT
└── yap.xcodeproj             # GENERATED, gitignored
```
...
### 5.5 Distribution
- Repo layout: SPM workspace + `project.yml` (XcodeGen) at root. Packages under `Packages/`. `yap.xcodeproj/` is gitignored — regenerable via `./bootstrap.sh`.
...
- Release workflow: tag `v0.x.y` → GitHub Actions installs xcodegen, runs `xcodegen generate`, builds `.app`, zips, attaches to release.
